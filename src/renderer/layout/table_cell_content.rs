...
use super::border_rendering::{
    build_cell_edge_masks, build_row_col_x, collect_cell_borders, render_edge_borders,
    ...
    pub(crate) fn layout_embedded_table(
                ...
                for (ctrl_idx, ctrl) in para.controls.iter().enumerate() {
                    match ctrl {
                        Control::Picture(pic) => {
                            ...
                            let img_data =
                                find_bin_data(bin_data_content, bin_id, self.is_hwpx_source.get())
                                    .map(|bd| bd.data.load());
        ...
        if self.show_transparent_borders.get() {
            ...
            table_node.children.extend(render_transparent_borders(
                tree, &h_edges, &v_edges, &h_mask, &v_mask, &row_col_x, &row_y, table_x, table_y,
            ));
        }
