        ...
        while cursor_row < row_count {
            ...
            let avail_for_rows = {
                ...
                let declared_one_row_ahead = declared_boundary.is_some_and(|(acc, next)| {
                    ...
                    } else if one_more > base + BOTTOM_SQUEEZE_TOLERANCE_PX {
                        // [R5-b] HWP5: plain 예산으로 담기는 행까지 선언 상한을 신뢰한다.
                        //   종전(#2391 v4)은 `one_more <= base`(plain)를 거부하고 [#2097] 쪽 하단
                        //   **압축만** 담으려는 행에 한정했다 — 「오라클 없는 문서를 움직이지 않는다」
                        //   규약을 지키려는 보수적 선택이었지, 한컴이 그렇다는 증거는 아니었다.
                        //   실측으로 그 전제가 깨졌다(docs/R5-b-…-20260906-1113.md):
                        //     · 그 「오라클 없는 4문서」 중 3편은 한컴 정본을 이미 갖고 있었고
                        //       hwpctl_API 는 78문서 sweep 세트 안에 Hwp 2022 정본과 함께 있었다.
                        //     · 정본 대조 11군집 전부 일치, 반례 0 —
                        //       kps-ai 1 · sijang 3(pi=723·1298·1320) · hwpctl_API 3(p62~64, 쪽 1:1) ·
                        //       편람 .hwp 3 · hwp-3.0-HWPML 1(CRD 한글 2024 로 직접 확인).
                        //     · 전부 같은 형상 — 종전 우리가 한 행(또는 한 제목)을 더 담았다.
                        //   게이트 문서 sijang-2017 은 142쪽이 움직인다 — 쪽 번호가 이미 어긋나 있어
                        //   (우리 p71 ↔ 정본 p72/73) 쪽 번호 기준 지표는 못 쓰고, 내용 기반 정렬로 재면
                        //   개선 140 · 악화 1(우리 p74, 0.9041→0.8699) · 무변화 1 이고 0.95 이상 일치 쪽이
                        //   22 → 157 이다. 「움직이면 정본 대조로 개선을 증명한다」를 충족한다.
                        //   ⚠ 이 수리는 ΣF 를 안 올린다 — 개선이 전부 F 접두(첫 어긋남) 뒤에 있다.
                        return false;
                    }
